            ...
            <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
                {currentList.map(item => (
                    <div key={item.id} className={`bg-black/20 border rounded-xl p-4 transition-all group ${isConnected(item.id) ? 'border-green-500/50' : 'border-card-border hover:border-primary/40'}`}>
                        ...
                        {isConnected(item.id) ? (
                            <button onClick={() => setExchanges(exchanges.filter(e => e.id !== item.id))} className="w-full py-2.5 rounded-lg text-xs font-bold bg-red-500/10 text-red-500 border border-red-500/30 hover:bg-red-500 hover:text-white transition-all">Desvincular</button>
                        ) : (
