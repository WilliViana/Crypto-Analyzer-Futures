...
import { ArrowUp, ArrowDown, History, Search, Download, Filter, Target, DollarSign } from 'lucide-react';
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 shrink-0">
                <div className="bg-surface border border-card-border rounded-xl p-4 shadow-lg flex items-center justify-between">
                    <div>
                        ...
                        <p className={`text-xl font-mono font-bold ${trades.reduce((a, b) => a + b.pnl, 0) >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                            ${trades.reduce((a, b) => a + b.pnl, 0).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <p className="text-xl font-mono font-bold text-purple-400">
                            {trades.filter(t => t.status === 'CLOSED').length > 0
                                ? Math.round((trades.filter(t => t.status === 'CLOSED' && t.pnl > 0).length / trades.filter(t => t.status === 'CLOSED').length) * 100)
                                : 0}%
                        </p>
                    </div>
                ...
                <div className="bg-surface border border-card-border rounded-xl p-4 shadow-lg flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xl font-mono font-bold text-green-400">
                            +${Math.max(0, ...trades.map(t => t.pnl)).toFixed(2)}
                        </p>
                    </div>
